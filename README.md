...
6. [ROI 계산 로직](#roi-계산-로직)
7. [리드 수집](#리드-수집)
8. [기술 스택과 아키텍처](#기술-스택과-아키텍처)
9. [프로젝트 구조](#프로젝트-구조)
10. [로컬 개발 및 미리보기](#로컬-개발-및-미리보기)
11. [배포](#배포)
12. [배포 검증](#배포-검증)
13. [디자인 시스템](#디자인-시스템)
14. [접근성·보안·개인정보](#접근성보안개인정보)
15. [운영 체크리스트](#운영-체크리스트)
16. [관련 문서](#관련-문서)
...
```
월 절감(저) = 건당시간 × 월건수 × 절감률(하한) × 시간당인건비
월 절감(고) = 건당시간 × 월건수 × 절감률(상한) × 시간당인건비
기대값 = (저 + 고) / 2
```
...
총점이 낮을수록 절감 실현이 어렵다고 가정합니다.

```
실현율 = 0.55 + ((총점 - 7) / (35 - 7)) × 0.40
```
...
```
월 순절감(정상화 후) = 보정 절감액 - 월 AI 운영비
```

- **투자 회수**: 초기 투자비 대비 누적 순현금흐름이 회수되는 시점 (최대 36개월 시뮬레이션)
- **12개월 ROI**: `(12개월 누적 순현금흐름 - 초기 투자비) / 초기 투자비 × 100`
...
| 업종 | 예시 업무 | 기본 절감률 |
|------|-----------|-------------|
| 마케팅 | 콘텐츠 초안, 주간 성과 리포트, 광고 소재 분석 | 0.65 (높음) |
| PR | 보도자료 초안, FAQ 응대, 취재 대응 자료 | 0.70 (높음) |
| B2B SaaS | 잠재고객 등급, 제안서 초안, 고객 문의 분류 | 0.55 (중간) |
| 커머스 | 상품 상세, 리뷰 분석, 프로모션 가설 | 0.60 (높음) |
| 중소기업 | 회의록, 주간 보고, 영업 리서치 | 0.50 (중간) |
...
```mermaid
flowchart LR
	A["반복 업무 입력<br/>(최대 3개)"] --> B["절감액 구간 계산<br/>low / high / expected"]
	C["성숙도 총점<br/>(7~35점)"] --> D["실현율 보정<br/>55% ~ 95%"]
	B --> E["보정 절감액"]
	D --> E
	E --> F["월 운영비 차감"]
	F --> G["도입 곡선 시뮬레이션<br/>M1:50% M2:75% M3+:100%"]
	G --> H["투자 회수 · 12개월 ROI"]
	H --> I["결과 화면 렌더링<br/>레이더 · 취약영역 · CTA"]
```
